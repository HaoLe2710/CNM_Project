...
public interface AiMessageRepository extends BaseRepository<AiMessage, Long> {
  ...
  @Query("""
          SELECT COUNT(m)
          FROM Message m
          JOIN MessageUserState mus ON m.id = mus.messageId
  ...
  @Query("""
                  ...
                  FROM Message m join MessageUserState mus ON m.id = mus.messageId join UserProfile up on up.userId = m.senderId
                  WHERE m.conversationId = :conversationId 
                    AND m.deletedAt IS NULL
                    AND m.senderId <> :userId
                    AND mus.userId = :userId 
  ...
  @Query("""
                  ...
                  limit 1
      """)
  MessageDto findFirstByConversationIdOrderByCreatedAtDesc(@Param("conversationId") UUID conversationId,
      @Param("userId") UUID userId);
}